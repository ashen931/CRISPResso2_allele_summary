...
get_run_info <- function(run_log_string){
  ...
  guide_seqs <- str_extract(run_log_string[1],"--guide_seq [ACGT[:punct:]]{6,} --aln_seed") %>% 
    str_remove( "--guide_seq ") %>% str_remove(" --aln_seed") 
  ...
  run_info <- data.frame(guide_seq = unlist(guide_seqs), ref_seq = rep(ref_seqs, n_guides), 
...
get_run_table <- function(sample_dir, crispresso_run_info, noSub){
  ...
  for(n in seq(1, nrow(crispresso_run_info))){
    ...
    # if(crispresso_run_info$q_window[n]%%2 == 0){
    #   q_window <- crispresso_run_info$q_window[n]
    # }else{
    #   q_window <- crispresso_run_info$q_window[n]+1
    # }
    q_window <- crispresso_run_info$q_window[n]
