...
get_run_info <- function(run_log_string){
  ...
  guide_seq <- str_extract(run_log_string[1],"-guide_seq [ACGT[:punct:]]{6,} --aln") %>% 
    str_remove( "-guide_seq ") %>% str_remove(" --aln")
  
  print(guide_seq) #ADDED
  
  #find the numeric index (order) of the desired allele in the amplicon name & sequeunce list
  ref_names <- str_extract(run_log_string[1],"--name [[:alnum:][:punct:]\\s]{1,} --n") %>%
    str_remove("--name ") %>% str_remove(" --n") %>% strsplit(split = ",")
  ref_name <- ref_names[[1]]
  
  print(ref_name) #ADDED
  ...
  #ADDED
  View(data.frame(guide_seq = guide_seq, ref_seq = ref_seq, ref_name = ref_name,
