#20190801_CRISPResso2_alleles_functs.R
# conda environment: py27_biocondR_env
# adapted from 20190521_CRISPResso2_alleles_qw_functs.R
# last modified: 2019_08_01
#
# This script contains all the functions required for 20190702_BE_crispresso2_analysis.R
# The functions are listed in the following order.

# Functions:
#     get_allele_frequency_table() 
#         get_run_info()
#         get_run_table()
#         reverse_complement() 
#         get_plot_allele_table_from_file()
...
#
# CALLS HELPERS: get_plot_allele_table_from_file(), select_alleles_for_plotting(),
...
#             desired_allele = string name (as appears in CRISPResso Allele_frequency.txt) of desired allele
...
get_allele_frequency_table <- function(crisp_out_dir, sample_dirs_list, desired_allele, percent_frequency, dataID){
  ...
  for(sample_dir in sample_dirs_list){ #loop through directories in sample_dirs_list
    ...
    crispresso_run_info <- get_run_info(run_log_string[1], desired_allele)
    
    #If the desired allele is not found among the reference sequences, print "XXX allele not found."
    if(typeof(crispresso_run_info) == "character"){
      
      print(crispresso_run_info)
      ...
      indel_table_unique <- get_run_table(sample_dir, crispresso_run_info, desired_allele)
  ...
  ranking_table <- filter_at(ranking_table, 
                             grep(dataID, names(ranking_table), value = TRUE), 
                             any_vars(. >= percent_frequency))
  
  ### 7. Order the alleles/rows by indel ####################################################################
  
  ordered_alleles_table <-order_indels(ranking_table)
...
#
# CALLS HELPERS: NA
# CALLED IN: get_allele_frequency_table()
# ARGUMENTS:  run_log_string = string extracted from CRISPResso2 RUNNING_LOG.txt containing run parameters
#             desired_allele = allele of interest; used to identify numeric index of the reference sequence
# OUTPUT: allele_table = data frame containing extracted CRISPResso parameters for each allele
get_run_info <- function(run_log_string, desired_allele){
  
  #extract the guide sequence
  guide_seqs <- str_extract(run_log_string[1],"--guide_seq [ACGT[:punct:]]{6,} --aln") %>% 
    str_remove( "--guide_seq ") %>% str_remove(" --aln")
  
  #find the numeric index (order) of the desired allele in the amplicon name & sequeunce list
  ref_names <- str_extract(run_log_string[1],"--amplicon_name [[:alnum:][:punct:]\\s]{1,} --") %>%
    str_remove("--amplicon_name ") %>% str_remove(" --") %>% strsplit(split = ",")
  ref_index <- grep(desired_allele, ref_names[[1]])
  
  #check to see if the desired allele is one of the reference amplicons. 
  # if so, extract reference sequence information.
  # if not, return the string "XXX allele not found." for printing in get_allele_frequency_table() 
  if(length(ref_index) == 0){
    
    return(paste(desired_allele, "allele not found.", sep = " ")) #return "XXX allele not found."
    
  }else{
    list_ref_seqs <- str_extract(run_log_string[1],"--amplicon_seq [ACGTatcg[:punct:]]{1,} --min") %>%
      str_remove("--amplicon_seq ") %>% str_remove(" --min") %>% strsplit(split = ",")
    #extract the amplicon reference sequence
    ref_seq <- list_ref_seqs[[1]][ref_index]
  }
  
  #extract the quantification window size (#bp around the predicted cut site)
  q_window <- str_extract(run_log_string[1],"--quantification_window_size [[:digit:]]{1,} --quant") %>% 
    str_remove("--quantification_window_size ") %>% str_remove(" --quant") %>% as.numeric()
  ...
  plot_window_size <- str_extract(run_log_string[1],"--plot_window_size [[:digit:]]{1,}") %>%
    str_remove("--plot_window_size ") %>% as.numeric()
  
  #return CRISPResso2 run/sample information as a data frame
  return(data.frame(guide_seq = guide_seqs, ref_seq = ref_seq, 
                    q_window = q_window, qw_center = qw_center, plot_window_size = plot_window_size * 2)) #ADDED "*2
}
...
## New function:
#
get_run_table <- function(sample_dir, crispresso_run_info, desired_allele){
  ...
  guide_seq_list <- str_split(crispresso_run_info$guide_seq[1], pattern = ",")[[1]] #may have >1 guide
  
  #get full reference sequence to align with guide_sequence to determine the cut site position
  ref_seq <-  toupper(crispresso_run_info$ref_seq[1])
  
  #if q_window is odd, CRISPResso2 automatically adds 1
  if(crispresso_run_info$q_window[1]%%2 == 0){
    q_window <- crispresso_run_info$q_window[1]
  }else{
    q_window <- crispresso_run_info$q_window[1]+1
  }
  qw_center <- crispresso_run_info$qw_center[1]
  plot_window_size <- crispresso_run_info$plot_window_size[1]
  ...
  ### frame. ################################################################################################
  guide_locs <- c() #initalize vector to hold guide locations
  for(n in seq(1,length(guide_seq_list))){
    
    if(regexpr(guide_seq_list[n], ref_seq)[1] < 0){
      ...
      cut_site <- regexpr(reverse_complement(guide_seq_list[n]), ref_seq) - qw_center
    }else{
      cut_site <- regexpr(guide_seq_list[n], ref_seq) + (str_length(guide_seq_list[n]) + qw_center)
    }
    
    guide_locs <- c(guide_locs, cut_site) #append cut_site of 1st bp in the guide
  }
  #organize guide info in a data frame for easy access
  ordered_guides <- data.frame(guide_seqs = guide_seq_list, cut_sites = guide_locs) 
  
  #order the guides by their proximity to the 1st bp in the amplicon reference
  #  This ensures that the guides will be analyzed in order, and the indels will be generated in order.
  ordered_guides <- ordered_guides[order(ordered_guides$cut_sites),]
  first_guide_cut <- ordered_guides$cut_sites[1]
  ordered_guides <- ordered_guides %>% mutate(difference = first_guide_cut - ordered_guides$cut_sites) %>%
    mutate(cut_index_adjustment = cut_minus1 - (str_length(guide_seq_list[n]) + qw_center) 
           + (first_guide_cut - ordered_guides$cut_sites))
  ...
  ### 3. generate one allele table for each guide ###########################################################
  for(n in seq(1,length(ordered_guides$guide_seqs))){ #loop through guides
    
    guide_seq <- ordered_guides$guide_seqs[n] #set current guide
    cut_index_adjustment <- ordered_guides$cut_index_adjustment[n] #set current index adjustment
    
    ### A. Extract information on desired_allele from CRISPREsso2 Alleles_frequency_table.txt ################
    allele_table <- get_plot_allele_table_from_file(sample_dir, guide_seq, desired_allele)
    
    #print information to check everything is correct
    print(paste("guide_seq:", guide_seq, sep = " "))
    print(paste("ref_seq:", ref_seq, sep = " "))
    print(paste("cut_minus1:", as.character(cut_minus1), sep = " "))
    print(paste("cut_index_adjustment:", as.character(cut_index_adjustment), sep = " "))
    print(paste("quantification_window_size:", as.character(q_window), sep = " "))
    print(paste("quantification_window_center:", as.character(qw_center), sep = " "))
    
    ### 4. Generate data frame that only contains alleles with valid editing patterns and includes their ###
    ### indel summaries in the "indel" column ##############################################################
    ...
    print("*************")
    print(sum(filter(allele_table, Unedited == TRUE)[,ncol(allele_table)], na.rm = TRUE))
    print(sum(filter(ranked_indel_table, Unedited == TRUE)[,ncol(ranked_indel_table)-2], na.rm = TRUE))
    print("*************")
    
    #join tables for each allele together so that there is only one table per run
    if(n == 1){
      #if the run_table is for the 1st guide, join all variables to the empty template data frame 
      # (the Aligned_Sequence & Reference_Sequence for this run will use that of the first guide)
      run_table <- right_join(run_table, ranked_indel_table, 
                              by = c("Aligned_Sequence", "Reference_Sequence", "Unedited",
                                     "n_deleted", "n_inserted", "n_mutated", "indel", "substitutions"))
    }else{
      #filter out the rows (reads) that have already been covered in the data tables of previous guides
      ranked_indel_table <- filter(ranked_indel_table, !(indel %in% unique(run_table$indel)))
      run_table <- rbind(run_table, ranked_indel_table)
    }
  }
  ...
  ### 5. Collapse ranked_indel_table duplicate indels so that there are no duplicate indels ######################################
...
unite_amplicon_indels <- function(run_table){
  
  run_table[is.na(run_table)] <- " "
  
  for(n in seq(1:nrow(run_table))){
    if(run_table$indel.x[n] != run_table$indel.y[n]){
      run_table$indel.x[n] <- paste(run_table$indel.x[n], run_table$indel.y[n], sep = " ")
    }
    if(run_table$substitutions.x[n] != run_table$substitutions.y[n]){
      run_table$substitutions.x[n] <- paste(run_table$substitutions.x[n], run_table$substitutions.y[n], sep = " ")
    }
  }
  return(run_table)
}
...
#Problems:
# -every guide assumes that all deleted base pairs outside of plot window belong to its associated
# deletion ("unseen" deletions may be split between 2 guides)

# - some bp characterizations are off
...
#             desired_allele = allele of interest; used to filter information extracted from 
#                              Alleles_frequency_table.txt
# OUTPUT: allele_table = data frame containing extracted CRISPResso read data for each allele
get_plot_allele_table_from_file <- function(sample_dir, guide_seq, desired_allele){
  ...
  read_name <-paste(sub("CRISPResso_on_", "", sample_handle[[1]][length((sample_handle[[1]]))]),"_percent_reads", sep = "")
  ...
  allele_table <- read.delim(file = filename, header = TRUE) %>%
    select(Aligned_Sequence, Reference_Sequence, Unedited, n_deleted, n_inserted, n_mutated, X.Reads.1) %>%
    ...
    rename_at("X.Reads.1",~read_name) #rename X.Reads.1 to match sample name
  return(allele_table)
}
...
select_alleles_for_plotting <- function(ranking_table, cut_minus1, cut_index_adjustment, ref_seq, q_window){
  ...
  ranked_indel_table <- get_indel_columns(ranking_table, cut_minus1, cut_index_adjustment, ref_seq, q_window) %>%
...
get_indel_columns <- function(ranking_table, cut_minus1, cut_index_adjustment, ref_seq, q_window){
...
check_editing_pattern <- function(allele_row, cut_minus1, edited, q_window){
  
  #calculate quantification window bp boundaries (based on plot allele sequeunce lengths)
  q_window_start <- cut_minus1 - q_window/2 + 1
...
get_deletion_summary <- function(ranking_table, n, cut_minus1, cut_index_adjustment, q_window){
  ...
  for(i in seq(1,length(del_starts))){
    ...
    if( (del_starts[i] + del_lengths[i]) >= q_window_start && del_starts[i] <=  q_window_end ){
      ...
      if (del_lengths[i] > 1){#if the deletion is longer than -1
...
#get the number of insertions from the plot allele table & insertion summary from the alignment
...
get_insertion_summary <- function(ranking_table, n, cut_minus1, cut_index_adjustment, q_window){
  ...
  #TO REMOVE
  #get the potential insertion start sites (find all start sites of "-" of deletion length in Reference_Sequence)
  #possible_insertion_starts <- gregexpr(paste(rep('-', ranking_table$n_inserted[n]), collapse = ""),
  #                                      ranking_table$Reference_Sequence[n])
...
get_substitution_summary <- function(ranking_table, n, cut_minus1, cut_index_adjustment, ref_seq, q_window){
...
#            data_indicator = some string within each of the sample column headings that identify columns 
...
collapse_duplicate_indels <- function(ranked_indel_table, data_indicator){
  ...
  #duplicate_indels_list <- unique(ranked_indel_table[duplicated(ranked_indel_table$collapsing_col),]$collapsing_col)
  ...
  #ranked_indel_table$duplicate <- duplicated(ranked_indel_table$collapsing_col)
  
  #get the first & last column indices of allele frequency data
  allele_freq_cols <- grep(data_indicator, names(ranked_indel_table), value = FALSE)
  ...
  while (num_row < nrow(ranked_indel_table)){
    ...
    if((ranked_indel_table$indel[num_row] %in% duplicate_indels_list)){
      #CHANGED HERE
      #if((ranked_indel_table$collapsing_col[num_row] %in% duplicate_indels_list)){
      ...
      if (length(allele_freq_cols) == 1){ #if there is 1 allele frequency column 
      ...
      }else{ #if there are > 1 allele frequency column/guides (called after final table is compiled)
...
# CALLS HELPERS: NA
# CALLED IN: CRISPResso2_heatmap.R
...
###### make_heatmap() #############################################################################
# This function is used generate a heat map from the data within plotting_matrix. It does not draw the 
# plot in R, but the plot will be saved to the CRISPRessoBatch output directory as a .jpeg file. The
# function can either plot linear or log10-transformed data depending on the log10 argument input. This 
# function is non-fruitful and does not return anything.
# CALLS HELPERS: draw_colnames_45()
# CALLED IN: CRISPResso2_heatmap.R
# ARGUMENTS:  plotting_matrix = matrix containing allele frequency data to be plotted in heatmap
#             plot_name = the first part of the exported plot file name
#             percent_frequency = minimum percent frequency alleles must have in at least 1 sample
#                                 in order to be included in the heatmap (included in exported file name)
#             log10 = boolean (Is the plotting_matrix data to be log10 transformed?)
#                     (default = FALSE)
make_heatmap <- function(plotting_table_unique, plot_name, percent_frequency, log10 = FALSE){
  #this section allows the heatmap column names to be drawn at a 45-degree angle
  assignInNamespace(
    x = "draw_colnames",
  ...
  #generate a matrix out of the plotting_table data frame 
  # (For the sake of heatmap plotting, exclude Aligned_Sequence, Reference_Sequence, Unedited, n_deleted, 
  # n_inserted, n_mutated, indel, match_editing_pattern, and mean & only include sample percent frequency 
  # columns.)
  plotting_table_frequencies <- plotting_table_unique[ , 8:(ncol(plotting_table_unique))]
  plotting_matrix <- data.matrix(plotting_table_frequencies[ , order(names(plotting_table_frequencies))])
  
  #make necessary variable settings depending on the type of data to go in the heatmap
  if(log10 == FALSE){ #if the plot data is to be linear, name plot accordingly and do not transform data
    plot_name <- paste(plot_name, "_", as.character(percent_frequency), ".jpeg", sep = "")
    plot_matrix <- plotting_matrix
  }else{ #if the plot data is to be log10 transformed, name plot accordingly and perform transformation
    plot_name <- paste(plot_name, "_log10_", as.character(percent_frequency), ".jpeg", sep = "")
    plot_matrix <- log10(plotting_matrix)
  }
  
  pheatmap(plot_matrix, 
           cluster_cols = FALSE,
           cluster_rows = FALSE,
           labels_row = plotting_table_unique$indel, #label rows with indel summaries
           labels_col = colnames(plotting_matrix), #label columns with sample names
           fontsize = 2, 
           cellwidth = 2,
           cellheight = 2,
           color =  colorRampPalette(brewer.pal(n = 7, name ="Blues"))(100), 
           na_col = "white", #NA values in the matrix are white
           border_color = NA, #no border between heatmap cells
           width = 30)#, #width of exported plot in inches
  #filename = plot_name) #name of exported file
}

###### draw_colnames_45() #############################################################################
# This function overwrites the default draw_colnames in the pheatmap package. The column names are
# drawn at a 45-degree angle on the heatmap to maximize space. Called in assignedInNamespace().
# Thanks to Josh O'Brien at http://stackoverflow.com/questions/15505607
# 
# CALLED IN: make_heatmap()
...
#recalculates percent frequencies of alleles so that they add to 100% for each sample
adjust_percent_frequency <- function(final_allele_table, dataID){
  
  data_cols <- grep(dataID, names(final_allele_table), value = TRUE)
...
############################################# UNUSED FUNCTIONS ###############################################
# get_adjusted_overlap <- function(table, overlap){
#   adjusted_overlap <- c()
#   for (n in seq(1, nrow(table))){
#     #because we only look at the Aligned_Sequence, only adjust for insertions because (deletions are
#     # replaced by "-" placeholders)
#     adjusted_overlap <- c(adjusted_overlap, overlap - table$n_inserted[n])
#   }
#   return(mutate(table, adjusted_overlap = adjusted_overlap))
# }