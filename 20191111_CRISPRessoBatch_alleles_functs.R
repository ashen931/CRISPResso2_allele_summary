#20191111_CRISPRessoBatch_alleles_functs.R
# conda environment: base
# adapted from 20191007_CRISPRessoBatch_alleles_functs.R
# last modified: 2019_10_31
#
# for use with CRISPResso version 2.0.31
#
# This script contains all the functions required for 20191031_BE_crispresso2_analysis.R
# Modified to take only one guide.
#
...
get_allele_frequency_table <- function(crisp_out_dir, sample_dirs_list, percent_frequency, dataID, noSub = TRUE){
  ...
  for(sample_dir in sample_dirs_list){ #loop through directories in sample_dirs_list
    ...
    crispresso_run_info <- get_run_info(run_log_string[1])
    ...
    if(typeof(crispresso_run_info) == "character"){
      ...
      indel_table_unique <- get_run_table(sample_dir, crispresso_run_info, noSub)
...
get_run_info <- function(run_log_string){
  
  #extract the guide sequence
  # guide_seq <- str_extract(run_log_string[1],"--guide_seq [ACGT[:punct:]]{6,} --aln_seed_len") %>% 
  #   str_remove( "--guide_seq ") %>% str_remove(" --aln_seed_len") 
  #ADDED
  guide_seq <- str_extract(run_log_string[1],"--guide_seq [ACGT[:punct:]]{6,} --aln_seed_len") %>% 
    str_remove( "--guide_seq ") %>% str_remove(" --aln_seed_len") %>% strsplit(split = ",")
  
  #find the numeric index (order) of the desired allele in the amplicon name & sequeunce list
  ref_names <- str_extract(run_log_string[1],"--name [[:alnum:][:punct:]\\s]{1,} --needleman_wunsch_gap_extend") %>%
    str_remove("--name ") %>% str_remove(" --needleman_wunsch_gap_extend") %>% strsplit(split = ",")
  ref_name <- ref_names[[1]]
  
  list_ref_seqs <- str_extract(run_log_string[1],"-amplicon_seq [ACGTatcg[:punct:]]{1,} --") %>%
  ...
  return(data.frame(guide_seq = guide_seq, ref_seq = ref_seq, ref_name = ref_name,
                    q_window = q_window, qw_center = qw_center, plot_window_size = plot_window_size * 2))
}



get_run_table <- function(sample_dir, crispresso_run_info, noSub){
  ...
  guide_seq <- crispresso_run_info$guide_seq # single guide only
  ...
  print(guide_seq)
  print(ref_seq)
  print(regexpr(guide_seq, ref_seq)[1])
  ...
  allele_table <- get_plot_allele_table_from_file(sample_dir, guide_seq)
...
get_plot_allele_table_from_file <- function(sample_dir, guide_seq){
...
get_indel_columns <- function(ranking_table, cut_minus1, cut_index_adjustment, ref_seq, q_window, noSub){
  ...
  #ADDED
  print(paste("q_window_start: ", q_window_start, sep = ""))
  print(paste("q_window_end: ", q_window_end, sep = ""))
